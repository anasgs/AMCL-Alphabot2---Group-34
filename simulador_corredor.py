...
MAP_SIZE_M = 15.8
WALL_THICKNESS_M = 1.78
ELEVATOR_HEIGHT_M = 6.0
ELEVATOR_WIDTH_M = 2.14
ELEVATOR_OFFSET_RIGHT_M = 2.0

# Conversões
map_cells = int(MAP_SIZE_M / RESOLUTION)
wall_cells = int(WALL_THICKNESS_M / RESOLUTION)
elevator_height = int(ELEVATOR_HEIGHT_M / RESOLUTION)
elevator_width = int(ELEVATOR_WIDTH_M / RESOLUTION)
elevator_offset_right = int(ELEVATOR_OFFSET_RIGHT_M / RESOLUTION)

grid = np.ones((map_cells, map_cells), dtype=int)  # Tudo começa como obstáculo = 1

# Área livre: corredor ao redor (moldura externa)
grid[:wall_cells, :] = 0  # Parte inferior
grid[-wall_cells:, :] = 0  # Parte superior
grid[:, :wall_cells] = 0  # Lado esquerdo
grid[:, -wall_cells:] = 0  # Lado direito

# Adiciona uma linha de obstáculo no topo, fundo, esquerda e direita (linha = parede fina)
grid[0, :] = 1  # Linha no topo
grid[-1, :] = 1  # Linha no fundo
grid[:, 0] = 1  # Linha na esquerda
grid[:, -1] = 1  # Linha na direita

# Área livre: elevador (retângulo interno no topo da parede interna)
elevator_top = wall_cells  # topo da parede interna
elevator_bottom = elevator_top + elevator_height
elevator_right = map_cells - wall_cells - elevator_offset_right
elevator_left = elevator_right - elevator_width

grid[elevator_top:elevator_bottom, elevator_left:elevator_right] = 0

GRID_LAYOUT = grid
GRID_ROWS, GRID_COLS = grid.shape
CELL_SIZE = 10  # pixels

# Define landmarks (4 cantos internos do quadrado oco)
landmarks = [
    (2, 0),  # canto superior esquerdo
    (0, 2),  # canto superior esquerdo
    (2, GRID_COLS-1),  # canto superior direito
    (0, GRID_COLS-3),  # canto superior direito
    (GRID_ROWS-3, 0),  # canto inferior esquerdo
    (GRID_ROWS-1, 2),  # canto inferior esquerdo
    (GRID_ROWS - 1, GRID_COLS - 3),  # canto inferior direito
    (GRID_ROWS - 3, GRID_COLS - 1),  # canto inferior direito
    
    # Cantos do retângulo do elevador
    (elevator_top, elevator_left-3),  # canto superior esquerdo do elevador
    (elevator_top+1, elevator_left-1),  # canto superior esquerdo do elevador

    (elevator_top, elevator_right+2),  # canto superior direito
    (elevator_top+1, elevator_right),  # canto superior direito

    (elevator_bottom-2, elevator_left-1),  # canto inferior esquerdo
    (elevator_bottom, elevator_left),  # canto inferior esquerd

    (elevator_bottom, elevator_right-1),  # canto inferior direito
    (elevator_bottom - 2, elevator_right)  # canto inferior direito
]
...
class SimulatorGUI:
    ...
    def step(self):
        ...
        if self.particles:
            ...
            mean_row, mean_col = self.get_estimated_position_best_80()
    ...
    def get_estimated_position_best_80(self):
        """Estimate position using the weighted average of the best 80% particles (by weight)."""
        if not self.particles:
            return None, None

        # Sort particles by weight (descending)
        sorted_particles = sorted(self.particles, key=lambda p: p.weight, reverse=True)
        n_best = int(0.8 * len(sorted_particles))
        best_particles = sorted_particles[:n_best]

        total_weight = sum(p.weight for p in best_particles)
        if total_weight == 0:
            # fallback: simple mean
            mean_row = np.mean([p.row for p in best_particles])
            mean_col = np.mean([p.col for p in best_particles])
        else:
            mean_row = sum(p.row * p.weight for p in best_particles) / total_weight
            mean_col = sum(p.col * p.weight for p in best_particles) / total_weight
    ...
    def show_violin_plot(self):
        ...
        # Simular vários passos para cada sigma
        num_steps = 20  # ou outro valor que faça sentido
        for sigma, label in [(1.0, "Motion Noise σ = 1.0m"), (3.0, "Motion Noise σ = 3.0m")]:
            self.sigma_motion.set(sigma)
            self.reset_particles()
            # Opcional: podes também resetar a posição do robô se quiseres
            errors = []
            for _ in range(num_steps):
                self.step()
                rr, rc = self.robot_pos
                errors.extend([abs(p.row - rr) + abs(p.col - rc) for p in self.particles])
    ...
    def generate_central_path(self):
        """Gera um caminho central pelo corredor e contorna o elevador."""
        path = []
        # Parâmetros do elevador já definidos no início do seu código
        # Use as variáveis elevator_top, elevator_bottom, elevator_left, elevator_right

        # 1. Percorre o corredor superior (centro do corredor)
        mid_row_top = elevator_top - 2
        for c in range(2, GRID_COLS - 2):
            if self.grid_map[mid_row_top, c] == 0:
                path.append((mid_row_top, c))
        # 2. Desce pelo lado direito até o topo do elevador
        for r in range(mid_row_top + 1, elevator_top):
            if self.grid_map[r, GRID_COLS - 3] == 0:
                path.append((r, GRID_COLS - 3))
        # 3. Contorna o elevador pela direita
        for r in range(elevator_top, elevator_bottom):
            if self.grid_map[r, elevator_right + 1] == 0:
                path.append((r, elevator_right + 1))
        # 4. Percorre a base do elevador (de direita para esquerda)
        for c in range(elevator_right + 1, elevator_left - 2, -1):
            if self.grid_map[elevator_bottom, c] == 0:
                path.append((elevator_bottom, c))
        # 5. Sobe pela esquerda do elevador
        for r in range(elevator_bottom - 1, elevator_top - 1, -1):
            if self.grid_map[r, elevator_left - 2] == 0:
                path.append((r, elevator_left - 2))
        # 6. Volta para o corredor superior (lado esquerdo)
        for r in range(elevator_top - 1, mid_row_top - 1, -1):
            if self.grid_map[r, 2] == 0:
                path.append((r, 2))
        # 7. Continua pelo corredor inferior (centro do corredor)
        mid_row_bottom = GRID_ROWS - 3
        for c in range(2, GRID_COLS - 2):
            if self.grid_map[mid_row_bottom, c] == 0:
                path.append((mid_row_bottom, c))
        # 8. Sobe pelo lado esquerdo até o ponto inicial
        for r in range(mid_row_bottom - 1, mid_row_top, -1):
            if self.grid_map[r, 2] == 0:
                path.append((r, 2))
        return path
    ...
    def start_auto_path(self):
        self.auto_path = self.generate_central_path()
